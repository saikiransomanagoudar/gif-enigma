...
export async function getRandomGame(
  ...
  try {
    ...
    // Early exit: if 10 valid games are found, stop checking more
    const targetValidGames = 10;

    // Batch fetch all game data first to reduce round trips
    const gameIds = gamesToCheck.map((item: any) =>
      typeof item === 'string' ? item : item.member
    );
    ...
    for (let i = 0; i < validGameCandidates.length; i++) {
      ...
      const isUserCreated =
        gameData.creatorId && gameData.creatorId !== 'anonymous' && gameData.creatorId !== 'system';
    ...
    if (recentGames.length === 0) {
      ...
      const hasPlayedAllGames =
        resolvedUsername && completedGames.length > 0 && totalGamesCount > 0;
...
export async function removeSystemUsersFromLeaderboard(
  _context: Context,
  ...
  try {
    ...
    const testAccounts = [
      ...
      'Mother_Play_6128',
      'Away-Airport-7318',
      'Beginning_Party_6899',
      'steve-sid',
      'seamless_ghost',
    ];

    const usernamesToClean = includeTestAccounts
      ? [...systemUsernames, ...testAccounts]
      : systemUsernames;
    ...
    for (const entry of globalLeaderboardEntries) {
      ...
      if (usernamesToClean.some((username) => id.includes(`:${username}`))) {
